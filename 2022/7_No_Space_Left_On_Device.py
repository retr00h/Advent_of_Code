...
# anyway?
#
# The device the Elves gave you has problems with more than just
# its communication system. You try to run a system update:
#
...
# The outermost directory is called /. You can navigate around the
...
#   g (size 2557), and h.lst (size 62596), plus file i indirectly (a contains
#   e which contains i).
    ...
    for line in f:
        ...
        if line != '$ ls':
            if line.startswith('$'):
                ...
                argument = line[2]
                if argument == '..':
                    tree_pointer = tree_pointer.father
                elif argument != '/':
                    for i in range(0, len(tree_pointer.children_directories)):
                        if tree_pointer.children_directories[i].name == argument:
                            tree_pointer = tree_pointer.children_directories[i]
                            break
            else:
                if line.startswith('dir'):
                    dir_name = line.split(' ')[1]
                    new_dir = Directory(n=dir_name, f=tree_pointer, fs=[], c=[])
                    tree_pointer.children_directories.append(new_dir)
                else:
                    line = line.split(' ')
                    size = int(line[0])
                    file_name = line[1]
                    new_file = {'name': file_name, 'size': size}
                    tree_pointer.files.append(new_file)


def part_one(root: Directory):
    total_size = 0
    for dir in root.children_directories:
        s = dir.size()
        if s <= 100000:
            total_size += s
        if dir.children_directories:
            total_size += part_one(dir)
    return total_size
...
# currently be 21618835, which isn't quite the 30000000 required by the update. Therefore,
# the update still requires a directory with total size of at least 8381165 to be deleted
# before it can run.
#
