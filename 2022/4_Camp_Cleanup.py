...
# from the ships, and so several Elves have been assigned the job
# of cleaning up sections of the camp. Every section has a unique
...
# Elves pair up and make a big list of the section assignments for each
...
with open(path.join(sys.path[0], "input\\4_Camp_Cleanup.txt")) as f:
  for line in f:
    line = line.strip()
    pairs.append(line)
...
def part_one(pairs):
  ...
  for pair in pairs:
    ...
    for i in range(0,2):
      for j in range(0,2):
        extrema.append(int(pair[i].split('-')[j]))
    if (extrema[0] <= extrema[2] and extrema[1] >= extrema[3]
        or extrema[0] >= extrema[2] and extrema[1] <= extrema[3]):
      res += 1
  return res

print("Part One: " + str(part_one(pairs))) # 567
...
def part_two(pairs):
  ...
  for pair in pairs:
    ...
    for i in range(0,2):
      for j in range(0,2):
        extrema.append(int(pair[i].split('-')[j]))
    if (extrema[0] <= extrema[2] and extrema[1] <= extrema[3] and extrema[1] >= extrema[2] or
        extrema[0] >= extrema[2] and extrema[1] >= extrema[3] and extrema[0] <= extrema[3] or
        extrema[0] <= extrema[2] and extrema[1] >= extrema[3] or
        extrema[0] >= extrema[2] and extrema[1] <= extrema[3]):
      res += 1
  return res

print("Part Two: " + str(part_two(pairs))) # 907