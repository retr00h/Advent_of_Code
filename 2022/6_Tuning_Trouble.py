...
# camp on foot and begin to make your way toward the star fruit
# grove.
# 
...
# but the most important one to set up right now is the communication
# system.
# 
...
with open(path.join(sys.path[0], "input\\6_Tuning_Trouble.txt")) as f:
  for line in f:
    line = line.strip()
    signal = line
    ...
    for k in range(0, 3):
      ...
      for l in range(k+1, 4):
        if tmp[l] == c:
          all_unique = False
          break
      if not all_unique: break
    if all_unique: return j
  return j

print("Part One: " + str(part_one(signal))) # 1034
...
print("Part Two: " + str(part_two(signal))) # 2472