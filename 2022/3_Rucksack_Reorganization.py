...
with open(path.join(sys.path[0], "input\\3_Rucksack_Reorganization.txt")) as f:
  for line in f:
    line = line.strip()
    compartment_one.append(line[0:len(line) // 2])
    compartment_two.append(line[len(line) // 2:])

def part_one(compartment_one, compartment_two):
  score = 0
  for i in range(0, len(compartment_one)):
    for letter in compartment_one[i]:
      if letter in compartment_two[i]:
        if letter.islower():
          score += (ord(letter) - 96)
        else:
          score += (ord(letter) - 38)
...
print("Part One: " + str(part_one(compartment_one, compartment_two))) # 8349
...
# a badge that identifies their group. For efficiency, within each group
# of three Elves, the badge is the only item type carried by all three Elves.
# That is, if a group's badge is item type B, then all three Elves will
# have item type B somewhere in their rucksack, and at most two of the
...
# the sticker attachment efforts: here, they are 18 (r) for the
...
def part_two(compartment_one, compartment_two):
  score = 0
  group = []
  for i in range(0, len(compartment_one ), 3):
    group.append(compartment_one[i] + compartment_two[i])
    group.append(compartment_one[i + 1] + compartment_two[i + 1])
    group.append(compartment_one[i + 2] + compartment_two[i + 2])
    letters = ''.join(OrderedDict.fromkeys(group[0] + group[1] + group[2]).keys())
