...
# you should be able to model the positions of the knots on a two-dimensional grid. Then, by following a
# hypothetical series of motions (your puzzle input) for the head, you can determine how the tail will move.
#
...
# one step, and so on. After each step, you'll need to update the position of the tail if the step means the
# head is no longer adjacent to the tail. Visually, these motions occur as follows (s marks the starting position
...
with open(path.join(sys.path[0], "input\\9_Rope_Bridge.txt")) as f:
    for line in f:
        line = line.strip()
        line = line.split(' ')
        direction = line[0]
...
def part_one(moves):
    head_row, head_col, tail_row, tail_col = 0, 0, 0, 0
    visited_positions = {str(tail_row) + ',' + str(tail_col)}
    for move in moves:
...
#
# The ropes are moving too quickly to grab; you only have a few seconds to choose how to arch your body to
# avoid being hit. Fortunately, your simulation can be extended to support longer ropes.
#
# Rather than two knots, you now must simulate a rope consisting of ten knots. One knot is still the head
# of the rope and moves according to the series of motions. Each knot further down the rope follows the knot
...
# and so it only visits 1 position. However, be careful: more types of motion are possible than before, so you
...
# .........########.........
#
# Simulate your complete series of motions on a larger rope with ten knots. How many positions does the
