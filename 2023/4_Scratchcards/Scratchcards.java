...
As you exit the gondola, the first thing you notice is that hte air here is
...
three times for each of the three matches after the first).
    ...
    - Card 4 has one winning number (84), so ti is worth 1 point.
...
public class Scratchcards {
    public static void partOne() {
        try {
            ...
            while (sc.hasNextLine()) {
                ...
                int[] numbers = Arrays.stream(halves[1].split("\\s+")).mapToInt(Integer::parseInt).toArray();
                halves = halves[0].split("\\s+");
                int id = Integer.parseInt(halves[1].replaceAll(":", ""));
                halves = Arrays.copyOfRange(halves, 2, halves.length);
                int[] winningNumbers = Arrays.stream(halves).mapToInt(Integer::parseInt).toArray();
                Card card = new Card(id, numbers, winningNumbers);
                int pow = card.getNumbersThatAreWinningNumbers().size();
    ...
    public static void partTwo() {
        try {
            ...
            for (Card c : cards) {
                int id = c.getId();
                int winningCards = c.getNumbersThatAreWinningNumbers().size();
                for (int i = id; i < id + winningCards; i++) {
