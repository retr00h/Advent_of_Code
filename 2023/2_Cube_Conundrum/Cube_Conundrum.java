...
public class Cube_Conundrum {
    public static void partOne() {
        try {
            Scanner sc = new Scanner(new FileInputStream("./2023/2_Cube_Conundrum/input/2_Cube_Conundrum.txt"));
            String line;
            int result = 0;
            ...
            while(sc.hasNextLine()) {
                ...
                int id = Integer.parseInt(halves[0].split("\\s")[1]);
                String[] games = halves[1].split(";\\s");
                int red = 0, blue = 0, green = 0;
                for (String game : games) {
                    String[] parts = game.split(",\\s");
                    for (String p : parts) {
                        ...
                        switch (color) {
                            case "red":
                                if (quantity > red) red = quantity;
                                break;
                            case "blue":
                                if (quantity > blue) blue = quantity;
                                break;
                            case "green":
                                if (quantity > green) green = quantity;
                                break;
                        }
                    }
                }
    ...
    public static void partTwo() {
        try {
            ...
            while(sc.hasNextLine()) {
                line = sc.nextLine();
                String[] games = line.split(":\\s")[1].split(";\\s");
                int red = 0, blue = 0, green = 0;
                for (String game : games) {
                    String[] parts = game.split(",\\s");
                    for (String p : parts) {
