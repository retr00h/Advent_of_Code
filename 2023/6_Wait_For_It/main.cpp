 ...
 *    is 200 millimeters.
 * 
 * Your toy boat has a starting speed of zero millimeters per second. For
 * each whole millisecond you spend at the beginninng of the race holding down
 * the button, the boat's speed increases by one millimiter per second.
 * 
 ...
 *    millimiters by the end of the race.
 ...
 *  - Hold the button for 4 milliseconds. After its remaining 4 milliseconds
 *    of travel time, the boat will have gone 12 millimeters.
 ...
 * ways you could win.
 * 
 * To see how much margin of error you have; determine the number of fways you
...
int partOne() {
    ...
    for (std::sregex_iterator iter(line.begin(), line.end(), digitRegex); iter != end; ++iter) {
        times.emplace_back(std::stoi(iter->str()));
    }

    // Process distance line
    std::getline(f, line);
    for (std::sregex_iterator iter(line.begin(), line.end(), digitRegex); iter != end; ++iter) {
        distances.emplace_back(std::stoi(iter->str()));
    }
    f.close();

    int res = 1;
    for (int race = 0; race < times.size(); race++) {
        int currentTime = times[race];
        int recordDistance = distances[race];
        int winningOutcomes = 0;
        for (int holdTime = 0; holdTime <= currentTime; holdTime++) {
            int speed = holdTime;
            int moveDistance = (currentTime - holdTime) * speed;
