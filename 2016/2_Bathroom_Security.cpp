...
string partOne(const vector<string> &instructions) {
  ...
  for (int i = 0; i < instructions.size(); i++) {
    ...
    for (int j = 0; j < instruction.length(); j++) {
      switch (instruction[j]) {
        case 'U': if (x - 1 >= 0) x--; break;
        case 'D': if (x + 1 <= 2) x++; break;
        case 'L': if (y - 1 >= 0) y--; break;
        case 'R': if (y + 1 <= 2) y++; break;
      }
    }
...
int main() {
  ...
  readInput(instructions);
    ...
    testInstructions.emplace_back("ULL");
    testInstructions.emplace_back("RRDDD");
